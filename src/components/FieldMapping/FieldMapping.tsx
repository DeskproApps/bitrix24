...
import productJson from "../../mapping/product.json";
...
export const FieldMapping = ({
  ...
  return (
    <Stack vertical gap={10} style={{ width: "100%" }}>
      ...
      {fields.map((field, i) => (
        <Stack vertical gap={10} style={{ width: "100%" }} key={i}>
          ...
          <Stack vertical style={{ width: "100%" }} gap={10}>
            {metadata?.map((metadataFields, i) => {
              ...
              if (usableFields.some((e) => e.value === "product")) {
                return (
                  <FieldMapping
                    key={i}
                    fields={
                      field.PRODUCTS.map((e: unknown) => ({
                        ...(e as Record<string, unknown>),
                        CURRENCY_ID: field.CURRENCY_ID,
                      })) ?? []
                    }
                    metadata={productJson.single}
                    hasCheckbox={true}
                  ></FieldMapping>
                );
              }
